...
Furthermore, data-driven induction of topics has proven quite successful and is in many respects a more objective way of organizing a collection of documents by content, as advocated in the \newcite{EAGLES1996} guidelines, among others.
...
% and Latent Dirichlet Allocation \cite{BleiEa2003}.
...
%, which is an expected result given the high dimensionality of the feature space (20 to 90 topics, see Section~\ref{sec:results}).
...
%The $\kappa$ statistic for the COW and DeReKo results from Table~\ref{tab:quality} is $\kappa_{\textrm{\tiny COW}}=0.575$ and $\kappa_{\textrm{\tiny DeReKo}}=0.582$, indicating that achieving a higher accuracy for the COW data is actually slightly harder than for the DeReKo data (see also the analysis of the confusion matrices below).
...
Moreover, as suggested by an anonymous reviewer, our results will have to be evaluated against a baseline classification that does not make use of LSI topics.
